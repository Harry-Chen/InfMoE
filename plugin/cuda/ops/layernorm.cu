...
template <typename T, typename U>
__global__ void cuApplyLayerNorm(T* __restrict__ output_vals, const T* __restrict__ vals, const int n1, const int n2,
                                 const U epsilon, const T* __restrict__ gamma, const T* __restrict__ beta) {
    // Assumptions:
    // 1) blockDim.x == warpSize
    ...
    for (auto i1 = blockIdx.y; i1 < n1; i1 += gridDim.y) {
        ...
        if (gamma != NULL && beta != NULL) {
            ...
            for (int i = thrx; i < n2; i += numx) {
                U curr = static_cast<U>(lvals[i]);
                ovals[i] = gamma[i] * static_cast<T>(c_invvar * (curr - mu));
            }
        } else if (beta != NULL) {
            for (int i = thrx; i < n2; i += numx) {
                U curr = static_cast<U>(lvals[i]);
                ovals[i] = static_cast<T>(c_invvar * (curr - mu)) + beta[i];
            }
        } else {
            for (int i = thrx; i < n2; i += numx) {
                U curr = static_cast<U>(lvals[i]);
...
__host__ void layernorm_kernel(T* __restrict__ output, const T* __restrict__ input, int n1, int n2, double epsilon,
    ...
    cuApplyLayerNorm<<<blocks, threads, nshared, stream>>>(output, input, n1, n2, U(epsilon), gamma, beta);
}
